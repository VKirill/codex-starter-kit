---
name: clean-code
description: Clean Code + Refactoring. Принципы Uncle Bob, SOLID, code smells, рефакторинг. Пиши чистый код и улучшай существующий.
stacks:
  - refactor
tags:
  - refactor
...
Loaded automatically when the description matches the active task. Specifics are documented in the body below.
...
При рефакторинге:
1. **Assess** — найди code smells, SOLID violations, risky hotspots
2. **Plan** — приоритизируй: High Impact + Low Effort → первым
3. **Execute** — маленькие слайсы, сохраняй поведение
4. **Verify** — тесты проходят, регрессий нет
...
| Large class (>200 lines) | Decompose by responsibility |
...
Подробные паттерны и примеры SOLID: `resources/refactoring-playbook.md`
