...
Текущая реализация находится в `plugins/claude-companion`. Runner использует adaptive tmux monitoring: сначала ждёт готовности интерактивного prompt в Claude, только потом отправляет review-pack, затем ждёт появления `outbox/<request_id>.md`, проверяет живость tmux-сессии, отслеживает активность pane и завершает работу по session exit, idle timeout или абсолютному safety cap. Если Claude напечатал marked review, но `Stop` hook не записал outbox, runner сохраняет ответ из tmux pane как fallback. Для ручных глубоких аудитов доступен режим `--wait-forever` с отдельным `--idle-timeout`.

Runner также ведёт fingerprint одинаковых review-запросов в `.codex/claude-bridge/requests/`. Если тот же mode/model/MCP/input/git status/verification context уже дал outbox, повторный запуск возвращает существующий результат без повторной отправки prompt в Claude. Если совпадающий запрос ещё выполняется, повторный запуск ждёт существующую `tmux`-сессию. `--force` нужен только для намеренного свежего review.
